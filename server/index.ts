...
import { scrapeDocuments } from './scraper.js';
...
app.post('/api/process', async (req, res) => {
  try {
    ...
    // Step 1: Scrape document metadata from Horsham portal
    const { documents, allRows } = await scrapeDocuments(reference);
    ...
    // Step 2: Use AI to summarize based on available data
    const result = await summarizeRepresentations(reference, documents, allRows);
